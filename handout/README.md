...
## Introduction
Please read the below instructions carefully, and when you are ready, 
your implementation will go to `lab/src`.

In these projects, you will build a linearizable, sharded key-value store with
...
of) these concepts, yet; the point of lecture and the projects is to help explain
those concepts to you.

We get there through five steps:
- Project 1: Investigate the framework code using a ping server.
- Project 2: Implement remote procedure call interface supporting many clients
  speaking to a server implementing a key-value store, with exactly-once
  semantics.
- Project 3: Design and implement a key-value primary-backup replication system for
  the key-value store, such that the system will continue to operate through
  (most) individual server failures.
- Project 4: Design and implement Paxos management of the key-value store, such that
  the system will continue to make progress provided that a majority of servers
  remain functional
- Project 5: Add sharding of the key-value store across server groups (each of which
  implements Paxos), with dynamic load balancing and atomic (transactional)
  multi-key updates across server groups.

Project 1 introduces the framework code and tools. Project 2 
demonstrates that you can modify the framework to add some simple functionality
that will be useful in later Labs. It is much easier than the other labs. Project 3
and 4 are of similar difficulty. Project 5 is somewhat harder than 3 and 4.
...
Also, git is highly recommended to use for these projects. It will also create a 
backtracking history for your implementations.

Please refer to the `environment` repo in our organization for a crash start, including VM setup, IDE usage, and Git usage.

In order to run the tests from the terminal, however, you also will use Python 3 and Make. Python 3 should be already included in Ubuntu 18.04.

## Framework Overview and Documentation
Project 1 provides a simple example of the framework code in action. 
You will be creating subclasses of the provided classes and should understand the contract those classes need to follow.
...
We have used Gradle for project builds. After running make commend, all dependencies will automatically downloaded for local usage. 
Thus, you will not encounter difficulties in dependencies. We will illustrate the detailed workflow in project 0.

We describe Lombok below. We believe that these libraries should meet your needs
while working through the projects, and when you submit your code, it must run
...
## Tests
To run tests, use the included `run-tests.py` script under the `build/handout` directory. 
It takes a number of command line arguments to modify the runtime environment that tests are running.
View them with `python3 run-tests.py --help`. Each of these arguments can also be set by
...
encouraged to reset the `tst` folders and execute `python3 run-tests.py` with the
default options before submitting.

If your code does not build successfully for tests to run, `make clean` will remove the build of your implementation, 
while `make clean-all` will remove all dependencies and build the code from scratch and see what the problems are.
...
Most tests in these projects come in two flavors: run tests and search tests.
...
(`python3 run-tests.py --single-threaded`) to be useful.
...
Running `python3 run-tests.py --checks` for a lab will attempt to report possible errors
...
report a certain error doesn't mean the problem does not exist.


## Debugging and Logging
Use of an IDE is strongly recommended; an IntelliJ project configuration is included in project files. 
Java has excellent support for debugging, even for multi-threaded applications.

Should you want to log any data, you can use the `lombok.extern.java.Log` package,
available in any class conveniently with the `@Log` Lombok annotation. An
argument to `python3 run-tests.py` will let you change the logging level. *IMPORTANT:*
Disable all logging statements before submitting your code (e.g., by only
logging at levels lower than `INFO`). We will include detailed code snippets in projects.

The framework has built-in logging of all delivered messages and timers at the
`FINEST` level (see it with `python3 run-tests.py -g FINEST`). This may be useful for
...
you can start any lab from its initial state by calling `python3 run-tests.py --lab N
...
`--start-viz` flag (e.g., `python3 run-tests.py --lab 2 --start-viz`). Once the
...
- Do not publicly publish your solutions or the skeleton code. *Always make the repo private.*
